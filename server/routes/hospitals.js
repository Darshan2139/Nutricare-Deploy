...
export const handleFindNearbyHospitals = async (req, res) => {
    try {
...
export const handleGetHospitalDetails = async (req, res) => {
    try {
        const { id } = req.params;
...
export const handleGeocodeAddress = async (req, res) => {
    try {
...
async function fetchGujaratHospitals() {
    try {
        const prompt = "List hospitals across all cities in Gujarat, India with fields: id (slug), name, address (city), phone (if known), website (if known), specialties (array), lat, lng, rating (0-5). Return JSON array only.";
